...
TOP_N               = 10         # keep top N answers; rest -> "Outros"
...
def cap_top_n_with_outros(counts: pd.Series, n: int = TOP_N, outros_label: str = "Outros") -> pd.Series:
    """
    Keep top n by count; sum the rest into an 'Outros' bucket (if any).
    Return counts including possible 'Outros'.
    """
...
def save_pie_jpg(values_pct: pd.Series, outfile: str):
    """Save a pie chart without title. values_pct are percentages (0–100) by answer."""
    if len(values_pct) == 0:
        return
...
def save_bar_jpg(values_pct: pd.Series, outfile: str):
    """Save a horizontal bar chart without title. All bars in BAR_COLOR.
    Ensures 'Outros' is always displayed at the bottom."""
    if len(values_pct) == 0:
        return

    # Move "Outros" to the end if present
    if "Outros" in values_pct.index:
        outros_val = values_pct["Outros"]
        values_pct = values_pct.drop("Outros")
        values_pct = pd.concat([values_pct, pd.Series({"Outros": outros_val})])

    # Reverse so that the first element goes at the top and 'Outros' ends up bottom
    ...
    bars = plt.barh(range(len(vals)), vals, color=BAR_COLOR)
...
with pd.ExcelWriter(OUTPUT_XLSX, engine="xlsxwriter") as writer:
    ...
    for raw_name in sheet_names:
        ...
        for col in df.columns:
            ...
            # ---- Choose chart type & save (no title) ----
